...
<?php    
    ...
    $searchUser = pg_prepare($conn, "user_search", "SELECT * FROM accounts WHERE username ILIKE $1");
    ...
    if (pg_num_rows($userResult) > 0) {
        ...
        while ($row = pg_fetch_assoc($userResult)) {
            ...
            echo "<section id='friendDisplay'><a href='../html/profile.php?id=$username'>$username</a></section><br>";
        }
    } else {
